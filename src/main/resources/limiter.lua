--[[
    ...
    进入Lua脚本的逻辑
        ...
        3. 判断当前时间窗口内的已请求数量 是否 小于 limit 限制次数
            a. 若小于，则把本次请求加入key中，以当前时间戳为分数score，以应用端生成的唯一请求标识为成员member，更新过期时间，本次请求可放行，后续可执行业务逻辑
...
local requestId = ARGV[4] -- 当前请求的唯一标识，避免并发请求覆盖同一个ZSet成员
...
if current < limit then
  -- requestId由应用端生成且每次请求唯一，确保ZADD不会覆盖同一毫秒内的其他请求
